...
| `awair-omni-local/` | Awair Omni 공기질 모니터의 로컬 API를 폴링해 온도, 습도, CO2, VOC, PM2.5, 조도, 소음, 그리고 로컬에서 계산한 EPA AQI를 가져옵니다. |
...
| `lotto645/` | 동행복권 로또 6/45 당첨번호를 매주 토요일 추첨 직후 자동으로 가져옵니다. 개편된 동행복권 당첨결과 페이지가 사용하는 조회 엔드포인트를 호출하며(구 `common.do` API는 현재 리다이렉트로 막혀 동작하지 않습니다), 등록해 둔 내 번호와 대조해 1~5등/낙첨까지 판정합니다. |
